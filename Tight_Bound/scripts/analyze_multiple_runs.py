...
def merge_results(dir: str, show_results: bool):
    ...
    try:
        for file in os.listdir("results/" + dir):
            if file.endswith(".json"):
                ...
                for j, round in enumerate(results["proposal_rounds"]):
                    if round:
                        proposals_round_times += round["proposal_round_time"]
        ...
        final_results["mean_proposals_round_times"] = np.mean(proposals_round_times)
        final_results["max_proposals_round_times"] = np.max(proposals_round_times)
        final_results["min_proposals_round_times"] = np.min(proposals_round_times)
        final_results["var_proposals_round_times"] = np.var(proposals_round_times)
