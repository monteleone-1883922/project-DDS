...
void process::handleMessage(cMessage *msg) {
    ...
    // SCOMMENTARE
     ...
     decided = -1;
     myNum = canIdecide(getIndex(),numSubmodules-2*infected,PV);
     recivedPV=0;
     collect(myNum);
     }
     }
    ...
    if (dynamic_cast<Decide*>(msg)) {
        ...
        if (recivedEv == numSubmodules) { //se ho ricevuto gli array da tutti
            ...
            for (int j = 0; j < numSubmodules; j++) {
                ...
                if (count0 > 2 * infected)
                    RV[j] = 0;
                if (count1 > 2 * infected)
                    RV[j] = 1;

            }
            int w = canIdecide(getIndex(), 2 * infected, RV);
            if (w != -1) {
                myNum = w;
            } else {
                ...
                prova = Ev[c];
                int b = canIdecide(getIndex(), 2 * infected, prova);
                if (b != -1) {
                    myNum = b;
                } else {
                    myNum = 0;
    ...
    if (dynamic_cast<Maintain*>(msg)) {
        ...
        int * result;
            ...
            EV << "Ho ricevuto tutte le proposte \n";
            decided = canIdecide(getIndex(),numSubmodules-2*infected,result);
            recivedMaintain=0;
            maintain(decided);
            }
    }
